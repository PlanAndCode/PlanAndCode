...
def showTrello(request):
    ...
    try:
        get_text=request.POST["cardIDDoing"]
        print(get_text)
        pc.moveCard(get_text,pc.getTrelloDoingList().id)
    except:
        print('')


    try:
        get_text=request.POST["cardIDBuild"]
        print(get_text)
        pc.moveCard(get_text,pc.getTrelloBuildList().id)
    except:
        print('')

    try:
        get_text = request.POST["cardIDDelete"]
        print(get_text)
        pc.deleteCard(get_text)
    except:
        print('')

    toDo = pc.getTrelloToDoCardsString()
    doing = pc.getTrelloDoingCardsString()
    build = pc.getTrelloBuildCardsString()
    test = pc.getTrelloTestCardsString()
    deploy = pc.getTrelloDeployCardsString()

    return render(request,"ShowTrello.html",{'ProjectName': ProjectName  ,   'toDo': toDo , 'doing' : doing, 'build' : build, 'test' :test, 'deploy': deploy})
